import {type CaptureManifest, type ProductBrief, type ScenePlan} from "@scenegraph/contracts";

/**
 * Pure scene-planning logic: turns a recorded product walkthrough into a
 * directed scene list. Deterministic composition — the same brief and capture
 * always produce the same narrative structure, camera moves, and timing.
 *
 * The only non-deterministic fields are the plan/scene UUIDs, which exist so
 * plans can be stored and referenced independently.
 */
...
export const direct = (projectId: string, brief: ProductBrief, capture: CaptureManifest): ScenePlan => {
